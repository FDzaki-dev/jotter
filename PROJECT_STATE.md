...
## [v2_Batch32] — 2026-08-28 (TERBARU)
User upload 2 file riset (`fitur_unggulan_colornote.md`, `perbandingan_colornote_google_keep.md`) — minta "merge permanent dengan MAIN_GOALS.md".
- **Dicek dulu**: isi ke-2 file TERNYATA overlap total — `fitur_unggulan_colornote.md` adalah subset PERSIS dari `perbandingan_colornote_google_keep.md` (5 section fitur ColorNote identik kata-per-kata; file kedua cuma nambah 1 tabel perbandingan vs Google Keep di akhir). Digabung jadi 1 section baru, bukan 2×copy-paste.
- **`MAIN_GOALS.md`** (diubah, 1 file): tambah section baru **"Referensi Kompetitor — ColorNote & Google Keep"** di paling bawah (setelah "Rekomendasi tes berikutnya", struktur/isi lama SAMA SEKALI TIDAK diubah — pure append). Isi: ringkasan pilar fitur ColorNote + status masing2 di Jotter (✅ ada / belum jadi goal / sengaja gak ada krn offline-only), plus tabel ColorNote-vs-Google-Keep dari file riset (dirapikan, ditambah kolom "Status di Jotter"). Ditutup catatan eksplisit: tabel ini **riset, BUKAN backlog otomatis** — item apapun (label/tag, lampiran media, pengingat lokasi, dst) baru jadi goal resmi kalau user eksplisit minta, supaya gak kesalahartikan sebagai pending queue baru batch depan.
- 2 file riset sumber **sengaja tidak disalin ke project** (bukan source code, bukan dependency build) — kontennya sudah permanen di `MAIN_GOALS.md`, sesuai permintaan "merge permanent".
- 1 file diubah, jauh di bawah micro-batch limit. Tidak ada resiko regresi kode (murni dokumentasi).
- **Pending Queue**: tetap kosong (section baru ini referensi, bukan task baru).

## [v2_Batch31] — 2026-08-28
Lanjut pending queue Batch30 — **Temuan #2: Versioning Lock**. User dikasih pilihan skema `versionName`, jawaban: **"versionName ikut auto juga (jadi 2.0.0-build42)"**.
- **`gradle.properties`** (diubah): tambah 1 properti baru `jotterBaseVersion=2.0.0` — satu2nya sumber kebenaran utk "epoch" semver (label generasi native-rewrite), dibaca BAIK oleh Gradle lokal (auto-load) MAUPUN CI (`grep` di `release.yml`). Ganti nilai ini nanti HANYA utk milestone besar (mis. pindah ke 3.0.0), bukan per-rilis — itu bukan "bump versi" yg dilarang Versioning Lock, krn gak nyentuh `versionCode`/build-identifier numeriknya.
- **`app/build.gradle.kts`** (diubah, Edit Parsial — cuma 2 baris `versionCode`/`versionName` di `defaultConfig`, sisa file 100% utuh): `versionCode = 1` (hardcoded) → `(findProperty("appVersionCode") as String?)?.toIntOrNull() ?: 1`. `versionName = "2.0.0"` (hardcoded) → `(findProperty("appVersionName") as String?) ?: "${...jotterBaseVersion...}-dev"`. Fallback `-dev` HANYA kepakai kalau build lokal tanpa CI (`gradle :app:assembleRelease` manual) — sengaja beda dari format CI (`-buildN`) biar APK lokal gak pernah ketuker APK rilis asli.
- **`.github/workflows/release.yml`** (diubah, Edit Parsial — 4 titik, sisa steps termasuk Stale Run Guard & Publish Release 100% utuh):
  1. Step **"Determine version identifiers"**: sumber `APP_VERSION` pindah dari `grep` ke `app/build.gradle.kts` (yg sekarang gak ada literal versionName lagi) → `grep` ke `jotterBaseVersion` di `gradle.properties`, lalu `APP_VERSION="${BASE_VERSION}-build${{ github.run_number }}"`.
  2. Step **"Build release APK"**: tambah flag `-PappVersionCode=${{ github.run_number }} -PappVersionName=${{ steps.ver.outputs.app_version }}` ke command `gradle :app:assembleRelease` — INI bagian yg benar2 nge-inject versi otomatis ke APK yg dikompilasi (sebelumnya versionCode/versionName CUMA dipakai buat penamaan file & release notes, TIDAK PERNAH benar2 masuk ke APK terkompilasi — gap asli yg dilaporkan user).
  3. Step **"Rename APK outputs"**: `app_version` sekarang SUDAH mengandung `-buildN` (dari langkah 1), jadi segmen `-build${{ github.run_number }}` yang tadinya ditambah terpisah di sini DIHAPUS (dulu bakal jadi duplikat "…-build42-build42…"). **Nama file APK akhir 100% IDENTIK byte-per-byte** dgn sebelumnya (`Jotter-v2.0.0-build42-arm64-v8a.apk`) — diverifikasi manual, 0 perubahan terlihat ke user/CI artifact naming.
  4. Step **"Publish GitHub Release"**: judul rilis `name:` disederhanain (`"Jotter ${{...app_version}} · build #${{...run_number}}"` → `"Jotter ${{...app_version}}"`, krn `app_version` udah termasuk `-buildN`, dulu bakal jadi redundan "2.0.0-build42 · build #42"). 3 baris nama file APK di body rilis disesuaikan (hapus segmen duplikat `-build${{ github.run_number }}`) biar teks di body PERSIS sama dgn nama asset asli (konsisten sama prinsip yg sama dipakai Batch9 dulu).
- **`UpdateChecker.kt` SENGAJA TETAP TIDAK DISENTUH** (sesuai rencana yg dicatat Batch30) — logic bandingin `tag_name` (bukan `versionCode`) masih 100% valid & gak bergantung sama sekali ke nilai `versionCode` yg sekarang jadi reliable; 0 resiko regresi ke fitur in-app updater.
- 3 file kode diubah (`gradle.properties`, `app/build.gradle.kts`, `.github/workflows/release.yml`) — pas di batas Micro-Batch (3).
- Belum diverifikasi CI/compile sungguhan (sandbox tanpa Android SDK/network) — sanity-check manual dilakukan: kurung/brace/parens balanced di ke-3 file (script python), simulasi `grep` ekstraksi `jotterBaseVersion` dari `gradle.properties` (hasil: `2.0.0` ✓), simulasi string filename akhir dgn `run_number=42` DIBANDINGKAN manual ke format lama — **identik persis**. **Prioritas cek di run CI pertama setelah push**: pastikan step "Build release APK" gak error soal `-P` flag (sintaks `-PappVersionCode=` harus nempel tanpa spasi ke value, sudah sesuai standar Gradle), dan buka Settings → "Cek Pembaruan" di app hasil rilis buat mastiin `currentVersionName` yg ditampilkan sekarang format `2.0.0-buildN` (bukan lagi cuma `2.0.0` polos) — ini PERUBAHAN TAMPILAN yg disengaja (imbas otomatis dari fix ini, user perlu tau: nomor versi yg keliatan di Settings & crash log field `Version` bakal ikut format baru ini juga).
- **🎉 Pending Queue KOSONG lagi** — kedua temuan dari audit sync Batch30 (Crash Logger `Version` field + Versioning Lock) sekarang CLOSED.

## [v2_Batch30] — 2026-08-28
User minta audit: "sinkronisasikan file dengan preferensi setting terkini" — cek seluruh project vs Core Protocol/Feature Locks di preferensi user saat ini, bukan dari laporan bug/fitur baru.
- **Temuan #1 (fixed batch ini) — Crash Logger kurang field `Version`**: spec Feature Lock ("Log WAJIB berisi Version, OS, Model, StackTrace") — isi log sebelumnya cuma ada OS/Device/StackTrace, field `Version` (versionName aplikasi) TIDAK PERNAH ditulis sama sekali.
  - **`CrashLogWriter.kt`** (diubah): `writeThrowable()` tambah parameter `appVersion: String`, baris pertama isi log sekarang `Version: $appVersion` (sebelum baris `OS:`). Tidak ada perubahan pada `write()`/`enforceRetention()` (path MediaStore, format nama file `crash_<ts>_<uuid>.txt`, FIFO 50 — semua tetap persis sama).
  - **`MainApplication.kt`** (diubah): baca `versionName` via `packageManager.getPackageInfo(packageName, 0).versionName` (pola PERSIS sama dgn `currentVersionName` yang sudah dipakai `SettingsScreen.kt` sejak batch awal — reuse pattern, 0 pendekatan baru) sebelum handler didaftarkan, lempar ke `CrashLogWriter.writeThrowable(...)` sbg argumen baru.
  - 2 file diubah, dalam batas micro-batch. Belum diverifikasi CI/compile (sandbox tanpa Android SDK — sanity-check manual: kurung/parenthesis balanced dicek via script, grep konfirmasi cuma 1 call-site `writeThrowable` di seluruh project & sudah cocok dgn signature baru).
- **Temuan #2 (BELUM dieksekusi — BLOCKER, nunggu jawaban user)** — **Versioning Lock dilanggar**: `app/build.gradle.kts` `versionCode = 1` & `versionName = "2.0.0"` masih HARDCODED, bukan otomatis dari `GITHUB_RUN_NUMBER` via CI (`.github/workflows/release.yml` cuma baca `versionName` yang sudah ada buat label build, gak pernah inject apapun balik ke Gradle).
  - **Kompleksitas**: ini bukan cuma soal lupa update — ada **keputusan desain terdokumentasi dari Batch9** (lihat entry lama di bawah) yang SENGAJA membiarkan `versionCode` hardcoded karena `UpdateChecker.kt` dirancang membandingkan `tag_name` rilis GitHub (string equality), BUKAN `versionCode` numerik — supaya feature "cek pembaruan" tetap jalan walau versionCode gak reliable. Preferensi terkini (Versioning Lock) rank di atas keputusan lama itu (Core Protocol > PROJECT_STATE.md di hierarki), TAPI eksekusi butuh 2 file Protected (`app/build.gradle.kts` + `.github/workflows/release.yml`) dan skema `versionName` otomatis dari run number belum jelas bentuknya (semver manual dipertahankan + versionCode aja yang auto? atau `versionName` ikut nempel run number juga?).
  - **Sengaja TIDAK dieksekusi turn ini** — sesuai Core Protocol rule "STOP → BLOCKER": informasi skema `versionName` kurang buat eksekusi aman di 2 file Protected sekaligus. Ditanyakan langsung ke user via pertanyaan pilihan.
  - **UpdateChecker.kt SENGAJA tidak disentuh** di rencana perbaikan manapun nanti (Zero-Unnecessary-Refactor) — logic tag-equality-nya tetap valid & gak bergantung ke nilai `versionCode`, jadi aman apapun skema yang dipilih user.
- **Pending Queue**: Temuan #2 di atas (Versioning Lock auto-bump `app/build.gradle.kts` + `.github/workflows/release.yml`) — nunggu jawaban user soal skema `versionName`, baru dieksekusi batch berikutnya.

## [v2_Batch29] — 2026-08-26
